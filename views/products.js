...
class Products {
	constructor(database) {
		...
		/* Tables */
		this._table                  = database.table('products');                  //Products
		this._table_group            = database.table('product_group');             //Product groups
		this._table_group_mapping    = database.table('product_group_mapping');     //Mapping between products and product groups
		this._table_location         = database.table('product_location');          //Product locations
		this._table_location_mapping = database.table('product_location_mapping');  //Mapping between products and product locations
		this._table_brand            = database.table('product_brand');             //Product brands
		this._table_package          = database.table('product_package');           //Product packages
		this._table_price            = database.table('product_price');             //Product prices
		this._table_stock            = database.table('product_stock');             //Product stock
		this._table_identifier       = database.table('product_identifier');        //Product identifiers
		this._table_identifier_type  = database.table('product_identifier_type');   //Types of product identifiers
	}
	...
	async list(session, params) {
		...
		var tasks = [
			Tasks.create('picture',      this._getFileAndSerialize.bind(this), products, 'picture_id'),
	...
	async edit(session, params) {
		...
		var dbTransaction = await this._database.transaction('Edit product #'+product.getIndex());
		try {
			await this.fillProductRecord(product, params, dbTransaction);
	...
	async remove(session, params) {
		var product = await this._findById(params);
		var dbTransaction = await this._database.transaction('Remove product #'+product.getIndex());
		try {
	...
	_getGroups(product_id) {
		return this._table_group_mapping.selectRecordsRaw('SELECT mapping.id as \'mapping_id\', group.id, group.name, group.description FROM `product_group_mapping` AS `mapping` INNER JOIN `product_group` AS `group` ON mapping.product_group_id = group.id WHERE `product_id` = ?', [product_id], false);
	}
	
	/* Product stock */

	_getStock(product_id) {
		return this._table_stock.selectRecordsRaw('SELECT `id`, `product_id`, `amount_initial`, `amount_current`, `timestamp_initial`, `timestamp_current`, `person_id`, `comment` FROM `product_stock` WHERE `product_id` = ? AND `amount_current` > 0 ORDER BY `timestamp_initial` ASC', [product_id], false);
	}
	...
	_getLocations(product_id) {
		return this._table_group_mapping.selectRecordsRaw('SELECT mapping.id as \'mapping_id\', location.id, location.name, location.description FROM `product_location_mapping` AS `mapping` INNER JOIN `product_location` AS `location` ON mapping.product_location_id = location.id WHERE `product_id` = ?', [product_id], false);
	}
	...
	async removeIdentifierType(session, params) {
		...
		var dbTransaction = await this._database.transaction('Remove identifier type #'+record.getIndex());
	...
	async removeLocation(session, params) {
		...
		var dbTransaction = await this._database.transaction('Remove location #'+record.getIndex());
	...
	async addStock(session, params) {
		...
		var dbTransaction = await this._database.transaction('Add stock');
		try {
	...
	async listGroups(session, params) {
		...
		var tasks = [
			...
			Tasks.create('picture',  this._getFileAndSerialize.bind(this), groups, 'picture_id')
		];
	...
	async createGroup(session, params) {
		...
		var dbTransaction = await this._database.transaction('Create product group');
		try {
	...
	async editGroup(session, params) {
		...
		var dbTransaction = await this._database.transaction('Edit product group #'+record.getIndex());
		try {
			if (typeof params.name !== 'undefined') {
				if (typeof params.name !== 'string') throw 'Expected \'name\' to be a string';
				record.setField('name', params.name);
	...
	async removeGroup(session, params) {
		...
		var dbTransaction = await this._database.transaction('Remove group #'+record.getIndex());
	...
	async listBrands(session, params) {
		...
		var tasks = [
			...
			Tasks.create('picture',  this.getFileAndDeserialize.bind(this), brands, 'picture_id')
		];
	...
	async createBrand(session, params) {
		...
		var dbTransaction = await this._database.transaction('Create product brand');
		try {
	...
	async editBrand(session, params) {
		...
		var dbTransaction = await this._database.transaction('Edit product brand #'+record.getIndex());
		try {
			if (typeof params.name !== 'undefined') {
				if (typeof params.name !== 'string') throw 'Expected \'name\' to be a string';
				record.setField('name', params.name);
	...
	async removeBrand(session, params) {
		...
		var dbTransaction = await this._database.transaction('Remove brand #'+record.getIndex());
	...
	async removePackage(session, params) {
		...
		var dbTransaction = await this._database.transaction('Remove package #'+record.getIndex());
