...
class Table {
    ...
    async list(where={}, separator="AND") {
        if (typeof where !== "object") {
            throw new Error("Query should be an object");
        }
        return this.selectRecords(where, "", separator, false);
    }

    async listExtra(where={}, extra="", separator="AND") {
        if (typeof where !== "object") {
            throw new Error("Query should be an object");
        }
        return this.selectRecords(where, extra, separator, false);
    }
    ...
    async selectRecords(whereParams,extra="", whereKeysSeparator="AND", asRecord=true) {
        ...
        for (var item in whereParams) {
            if ((typeof whereParams[item] === "string") || (typeof whereParams[item] === "number") || (typeof whereParams[item] === "boolean")) {
                ...
                if (whereParams[item].length < 1) {
                    throw new Error("Query contains empty array");
                }
                ...
                for (let operator in whereParams[item]) {
                    if (typeof operator !== "string") {
                        throw new Error("Invalid operator type");
                    } else if ((operator==="=") || (operator==="<=>") || (operator===">") || (operator===">=") || (operator==="IS") || (operator==="IS NOT") || (operator==="<") || (operator==="<=") || (operator==="LIKE") || (operator==="!=") || (operator==="<>") || (operator==="NOT LIKE") ) {
                ...
                throw new Error("Invalid where value type:" + (typeof whereParams[item]));
            }
        }
        ...
        try {
            let [rows, fields] = await this._opts.db._executeQuery(sql, whereValues);
            var result = [];
            for (var i = 0; i < rows.length; i++) {
                if (asRecord) {
            ...
            if (error.code == "ER_NO_SUCH_TABLE") {
                console.error("Error: table '"+this._opts.table+"' does not exist.");
                throw new Error("table '"+this._opts.table+"' does not exist");
            } else {
                this._opts.db._errorHandler(error);
    ...
    async selectRecordsRaw(sql, params, asRecord=true) {
        try {
            var result = [];
            let [rows, fields] = await this._opts.db._executeQuery(sql, params);
...
class Database {
    ...
    async _selectRpcMethod(params) {
        ...
        let records = await table.selectRecords(where);
        var result = [];
    ...
    registerRpcMethods(rpc, prefix) {
        ...
        rpc.addMethod(prefix+"select", _selectRpcMethod.bind(this));
    }
    ...
    _executeQuery(query, opts, transaction=null) {
        if (transaction !== null) {
            transaction.logQuery(query, opts);
            if (transaction.connection === null) {
                transaction.message("No connection?!");
                throw new Error("No connection in transaction");
            }
