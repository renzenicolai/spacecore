	...
	execute(session, params) {
		if (!("person_id" in params)) {
			return new Promise(function(resolve, reject) {
				return reject("Please provide a person_id in params.");
			});
		}
		if ((!("products" in params)) && (!("other" in params))) {
			return new Promise(function(resolve, reject) {
				return reject("Please provide products or other rows in params.");
			});
		}
		
		//console.log("Executing transaction for person",params.person_id);
			
		return this._opts.persons.list(session, {"id": params.person_id}).then((result) => {
			...
			var person = result[0];
			return this._opts.persons.getRecord(result[0].id).then(person_record => {
				...
				if ("products" in params) {
					for (var i in params.products) {
						if (typeof params.products[i] === "number") {
							...
							/*if (!(params.products[i].id in product_stock_request)) {
								product_stock_request[params.products[i].id] = [];
							}*/
							...
							return new Promise(function(resolve, reject) {
								return reject("Products should be either the product id or an object: {'id':..., 'amount':..., 'stock':...}");
							});
						}
					}
				}
				...
				return Promise.all(product_promises).then((product_results) => {
					...
					for (var product in product_results) {
						...
						//console.log("STOCK PROMISE FOR",product_id);
